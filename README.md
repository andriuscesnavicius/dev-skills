...
### App Testing
- **Dev Application Run**
  - Open app visual studio solution file locaten in 'app\App.sln'.
  - Run app by clicking on 'IIS Express'.
- **API**
  - Documentation can be found on this **[link](https://localhost:44303/index.html)** (app must run locally).
- **Database**
  - Test accounts will be generated on every new app run. 
  - Edit 'app\Modules\Account\Account.API\Data\accounts.json' to change test accounts.
- **Tests**
  - App should pass all tests on first test run.
  - App should be restarted for every test run since database receives new data after tests.
